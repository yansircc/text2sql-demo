...
export const pipelineComparisonRouter = createTRPCRouter({
	compareFullPipeline: publicProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			const testSchema = input.databaseSchema || JSON.stringify({
				companies: {
					columns: {
			...
			const [original, simplified] = await Promise.all([
				runOriginalPipeline(ctx, input.query, testSchema, input.vectorizedFields),
				runSimplifiedPipeline(ctx, input.query, testSchema, input.vectorizedFields)
			]);

			// Calculate improvements
			const improvements = {
				analysisTime: ((original.times.analysis - simplified.times.analysis) / original.times.analysis * 100).toFixed(1),
				schemaTime: ((original.times.schema - simplified.times.schema) / original.times.schema * 100).toFixed(1),
				sqlTime: ((original.times.sql - simplified.times.sql) / original.times.sql * 100).toFixed(1),
				totalTime: ((original.times.total - simplified.times.total) / original.times.total * 100).toFixed(1),
				
				analysisFields: ((original.complexity.analysisFields - simplified.complexity.analysisFields) / original.complexity.analysisFields * 100).toFixed(1),
				schemaFields: ((original.complexity.schemaFields - simplified.complexity.schemaFields) / original.complexity.schemaFields * 100).toFixed(1),
				sqlFields: ((original.complexity.sqlFields - simplified.complexity.sqlFields) / original.complexity.sqlFields * 100).toFixed(1),
			};
			...
			return {
				...
				original: {
					...original,
					sql: original.sql?.substring(0, 200) + (original.sql?.length > 200 ? "..." : "")
				},
				simplified: {
					...simplified,
					sql: simplified.sql?.substring(0, 200) + (simplified.sql?.length > 200 ? "..." : "")
				},
				improvements,
				recommendation: improvements.totalTime > "20" ? 
...
async function runOriginalPipeline(ctx: any, query: string, schema: string, vectorizedFields?: Record<string, string[]>) {
	...
	try {
		...
		if (analysisResult.analysis.routing.strategy === "rejected") {
			return { times, complexity, strategy: "rejected", error: analysisResult.analysis.feasibility.reason };
		}
		...
		return { times, complexity, strategy: analysisResult.analysis.routing.strategy };
		
	} catch (error) {
		console.error("Original pipeline error:", error);
		return { times, complexity, error: error.message };
	}
}

async function runSimplifiedPipeline(ctx: any, query: string, schema: string, vectorizedFields?: Record<string, string[]>) {
	const times = { analysis: 0, schema: 0, sql: 0, total: 0 };
	const complexity = { analysisFields: 0, schemaFields: 0, sqlFields: 0 };
	...
	try {
		...
		if (analysisResult.analysis.routing.strategy === "rejected") {
			return { times, complexity, strategy: "rejected", error: analysisResult.analysis.reason };
		}
		...
		if (analysisResult.analysis.sqlTables) {
			...
			const schemaResult = await ctx.schemaSelectorSimplified.selectSimple({
				...
				databaseSchema: schema,
				vectorIds: analysisResult.analysis.vectorQueries ? [1, 2, 3] : undefined // Mock IDs
			});
			times.schema = Date.now() - schemaStart;
			complexity.schemaFields = countFields(schemaResult.result);
			...
			const sqlStart = Date.now();
			const difficulty = analysisResult.analysis.routing.confidence > 0.8 ? "easy" : "hard";
			const sqlResult = await ctx.sqlBuilderSimplified.buildSimple({
				query,
				...schemaResult.result,
				difficulty,
				vectorIds: analysisResult.analysis.vectorQueries ? [1, 2, 3] : undefined
			});
			times.sql = Date.now() - sqlStart;
			complexity.sqlFields = countFields(sqlResult.result);

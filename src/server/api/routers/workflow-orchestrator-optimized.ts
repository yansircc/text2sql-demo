    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        const analysisResult = await api.queryAnalyzer.analyze({
          ...
          vectorizedFields: schemaRegistry.getSchema(schemaRef.schemaId)?.metadata?.vectorizedFields
        });
        ...
        if (strategy === "vector_only") {
          ...
          return {
            ...
            data: vectorResult.result.results.map(r => ({
              ...r.payload,
              _score: r.score,
        ...
        if (strategy === "sql_only" || strategy === "hybrid") {
          ...
          const filteredSchema = schemaRegistry.getFilteredSchema(schemaRef.schemaId, tables);
          
          // Parallel execution for hybrid mode
          if (strategy === "hybrid") {
            console.log("[Optimized] Executing hybrid search with maximum parallelization");
            ...
            if (vectorResults) {
              queryContextManager.update(queryId, {
                vectorContext: {
                  ...
                  ids: vectorResults.result.results.map(r => r.id),
                  topMatches: vectorResults.result.results.slice(0, 5).map(r => ({
                    id: r.id,
                    score: r.score,
